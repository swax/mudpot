...
function handleScanner(
  ...
  if (!state.identified) {
    ...
    if (data[0] === 0x16 && data[1] === 0x03) {
      ...
      delayedEnd(
        socket,
        "The lower levels do not answer in that language. Try the standard channel.\n",
      );
      return true;
    }
    // SSH
...
<form method="POST" action="/terminal"><input name="cmd" placeholder="Enter command..."><button>Submit</button></form>
    ...
    if (/^MGLNDD_/.test(probe)) {
      ...
      delayedEnd(
        socket,
        "The station does not recognize your signal. The corridor remains silent.\n",
      );
      return true;
    }
  }
  ...
  if (state.mode) {
    ...
    if (state.mode === "ftp") {
      ...
      if (/^PASS\s/i.test(line))
        delayedWrite(
          socket,
          "230 Welcome to Grey Sector. You are at Junction G-17. Try LIST.\r\n",
        );
      ...
      else if (/^RETR/i.test(line))
        delayedWrite(
          socket,
          "150 Opening connection.\r\nPower fluctuations in Grey Sector. Access crystal required for Records.\r\n226 Transfer complete.\r\n",
        );
      else if (/^PWD/i.test(line))
        delayedWrite(socket, '257 "/station/grey-sector/g-17"\r\n');
      else if (/^CWD/i.test(line))
        delayedWrite(socket, "250 You move deeper into the station.\r\n");
      else if (/^SYST/i.test(line))
        delayedWrite(socket, "215 BabylonStation Grey Sector\r\n");
      else if (/^QUIT/i.test(line))
        delayedEnd(
          socket,
          "221 You find your way back to the upper levels. Safe travels.\r\n",
        );
      else
        delayedWrite(
          socket,
          "500 Unknown command. Try LIST, RETR, or PWD.\r\n",
        );
