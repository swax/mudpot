...
function assertNotIncludes(str, substr, msg) {
  assert(!str.includes(substr), msg || `expected output not to include "${substr}"`);
}
...
assert(session.inventory.includes('access_crystal'), 'access_crystal in inventory');
...
assert(session.inventory.includes('security_seal'), 'security_seal in inventory');
